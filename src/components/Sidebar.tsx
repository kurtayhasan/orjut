...
export default function Sidebar({ className }: { className?: string }) {
  ...
  const { isSidebarOpen, setIsSidebarOpen, userRole, isPremium } = useAppContext();
  ...
  return (
    <>
      ...
      {isSidebarOpen && (
        <div 
          className="fixed inset-0 bg-black/50 z-[var(--z-sticky)] lg:hidden animate-fade-in" 
          onClick={() => setIsSidebarOpen(false)}
        />
      )}
      
      <aside className={cn(
        "fixed inset-y-0 left-0 z-[var(--z-sticky)] w-60 bg-[#1B2E1C] flex flex-col shrink-0 h-full transform transition-transform duration-300 lg:relative lg:translate-x-0",
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 custom-scrollbar">
          <nav className="flex flex-col gap-1.5">
            ...
            {userRole === 'engineer' && (
              <SidebarItem href="/dashboard/clients" icon={Users} label="Müşterilerim" active={pathname.includes('/clients')} />
            )}
            {userRole === 'admin' && (
              <SidebarItem href="/admin" icon={Shield} label="Sistem Yönetimi" active={pathname === '/admin'} />
            )}
          </nav>
        </div>
        ...
        <div className="p-4 border-t border-white/5 space-y-2">
          ...
          <div className="flex items-center gap-3 px-3 py-3 rounded-xl bg-white/5 mb-2">
            <div className="w-10 h-10 rounded-full bg-primary-light flex items-center justify-center text-[#1B2E1C] font-black text-sm">
              U
            </div>
            <div className="flex flex-col min-w-0">
              <span className="text-sm font-bold text-white truncate">Çiftçi</span>
              <span className={cn(
          ...
          <SidebarItem href="/dashboard/settings" icon={Settings} label="Ayarlar" active={pathname.includes('/settings')} />
          <button
...
function SidebarItem({ icon: Icon, label, href, active, badge }: { icon: any, label: string, href: string, active?: boolean, badge?: string }) {
  return (
    <Link 
      href={href} 
      className={cn(
